...
def DiscoveryMain(Framework):
    ...
    try:
        try:
            ...
            processes = processDiscoverer.discoverProcessesByCommandLinePattern('com.ibm.ws.runtime.WsServer')
            ...
            for configDirPath, runtimes in runtimesByConfigDirPath.items():
                    ...
                    for server in servers:
                        ...
                        if serverRuntime or server.hasRole(jee.AdminServerRole):
                            ...
                            if serverRuntime:
                                ...
                                server.jvm = (jee_discoverer.JvmDiscovererByShell(shell, None).
                ...
                if cell.isDistributed() and not administrativeServer:
                    ...
                    for node in nodes:
                        ...
                        if adminServers:
                            ...
                            administrativeServer.nodeName = node.getName()
                ...
                # determine whether we have at least one server with resolved IP address
                if not findFirst(lambda server: server.ip.value(),
                                 serverByFullName.values()):
                    logger.warn("%s and related topology won't be reported \
                    as there is no at least one server with resolved IP address")
                    continue

                sendVectorWithDomain = curry(sendTopologyWithDomainVector,
                                            Framework, _, domainVector
                )
                # discover resources
...
def discoverApplicationsInDomain(cell, cellLayout, fs, shell, parser, reporterCreator,
    ...
    appDiscoverer = jee_discoverer.BaseApplicationDiscovererByShell(shell, applicationLayout, descriptorParser)
    ...
    for server in serverByFullName.values():
        ...
        for app in (appServerRole and appServerRole.getApplications()) or ():
            ...
            try:
                ...
                application = appDiscoverer.discoverEarApplication(app.getName(), appDeploymentDirPath)
                        ...
                        if files:
                            ...
                            try:
                                if isinstance(module, jee.WebModule):
                                    descriptor = descriptorParser.parseWebApplicationDescriptor(file.content)
                                elif isinstance(module, jee.EjbModule):
                                    descriptor = descriptorParser.parseEjbModuleDescriptor(file.content)
                                else:
