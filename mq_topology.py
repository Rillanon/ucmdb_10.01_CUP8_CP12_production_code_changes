...
qTypeMap = {'QALIAS':'Alias Queue', 'QLOCAL':'Local Queue', 'QREMOTE':'Remote Queue', 'XMITQ':'Transmission Queue', 'SYSTEM':'System Queue', 'OTHER':'Other'}
...
## Get remote port from connection name of MQ channels
##############################################
def getMqPortAndIp(mqOSH, hostOSH):
    try:
        ...
        if mqListenerIP:
            ...
            ipOSH = modeling.createIpOSH(mqListenerIP)
...
## Get Queue Managers
##############################################
def getQManagers(shell, ipAddress, containerHostOSH, localFramework):
    try:

        logger.debug('getQManagers - ipAddress is ', ipAddress)
        returnDict = {}

        # qManagers = shell.execAlternateCmds(SUDO + 'dspmq', 'dspmq')
        qManagers = shell.execAlternateCmds(SUDO + 'dspmq', 'dspmq', '. /usr/mqm/bin/setmqenv -s;dspmq') # updated by Daniel La 17/02/14
        ...
        while match.find() == 1:
            ...
            mqOSH.setAttribute('application_version_number', mqVersionNumber)
            mqOSH.setAttribute('application_version', mqVersionString.strip())
            mqOSH.setAttribute('application_ip', ipAddress)
            #mqOSH.setAttribute('application_username', localClient.getUserName())
            #mqOSH.setAttribute('application_timeout', localClient.getTimeout())
            ...
            mqOSH.setContainer(containerHostOSH)
            ...
            if isValidString(qManagerName):
                ...
                if qManagerStatus.lower() == 'running':
                    ...
                    qMgrPortDetails = runMqscCommand(shell, 'DISPLAY LSSTATUS(*) ALL', qManagerName) # Daniel La - removed double quotes otherwise fails. 27/05/2013
                    if isValidString(qMgrPortDetails):
                        portstring = getBufferParameter(qMgrPortDetails, 'PORT') or ''
                        qMgrPort = int(portstring)
                        ip6ip = getBufferParameter(qMgrPortDetails, 'IPADDR') or ''





                        if ip6ip and ip6ip.strip() != "" and ip6ip != "*" and ip6ip != "::":
                            if ip6ip.find(":") != -1 :
                                qMgrIp = ip6ip.split(":")[-1]
                                mqOSH.setAttribute('application_ip', qMgrIp)




                            elif netutils.isValidIp(ip6ip):
                                mqOSH.setAttribute('application_ip', ip6ip)
                        mqOSH.setAttribute('application_port', qMgrPort)
                        mqListenerPort = qMgrPort
                        qManagerOSH.setAttribute('mqqueuemanager_listenerport', str(qMgrPort))
            ...
            for queue in queues:
                ...
                ## If this is a REMOTE Q, link it with its TRANSMIT Q
                    ...
                    if isValidString(xmitQ) and isValidString(remoteQMgrName) and isValidString(DISCOVER_REMOTE_HOSTS) and DISCOVER_REMOTE_HOSTS.strip().lower() in ['true', 'yes', 'y', '1']:
                        ...
                        ipserverOSH = None
                        remoteMqOSH = None
                        remoteHostDetails = runMqscCommand(shell, 'DISPLAY CHANNEL(*) WHERE(xmitq EQ ' + xmitQ + ') TYPE(SDR) CONNAME', qManagerName)
...
def DiscoveryMain(Framework):
    ...
    try:
        ...
        qManagerOshDict = getQManagers(shell, client.getIpAddress(), hostOSH, Framework)
        if qManagerOshDict != None and len(qManagerOshDict) > 0:
            for qManagerName in qManagerOshDict.keys():
                ...
                if qManagerName.startswith('ibm_mq_software'):
                    OSHVResult.addAll(getMqPortAndIp(qManagerOshDict[qManagerName], hostOSH))
                    continue
                ...
                #
                # modified by Daniel La 26/02/14 so we do not discover anything deeper than Queue Managers. This is because
                # the code tries to discover remote Queue Managers and during reconciliation results get muddled up.
                # In addition, at this time IAG do not care about anything deeper than the Queue Manager level.
                if isValidString(qManagerStatus) and qManagerStatus.lower() == 'JDS':
