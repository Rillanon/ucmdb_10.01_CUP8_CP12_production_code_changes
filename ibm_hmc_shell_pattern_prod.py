...
class LParDiscoverer(ibm_hmc_lib.GenericHmc):
        ...
        if managedSystem:
            ...
            for (lparId, lpar) in lpars.items():
                if lparId and lpar:
                    ...
                    if lpar.lparProfile: # modified by Daniel La to put in check for valid IP.
                        hostIp = None
                        if lparsIp.get(lparId) and re.match('\d+\.\d+\.\d+.\d+', lparsIp.get(lparId)):
                            hostIp = lparsIp.get(lparId)
                        elif lpar.rmcIp:
                            hostIp = lpar.rmcIp
                        lpar.lparProfile.lparIpAddress = hostIp
    ...
    def _getIps(self, managedSystem):
        ...
        return self._parsePartitionsIps(output)
...
def createHmcHostOsh(hmcHost ,ipAddress):
    ...
    if hmcHost and ipAddress:
        hostOsh = modeling.createHostOSH(ipAddress, 'host', None, hmcHost.hostname)
        ...
        if hmcHost.hmcSoftware:
            ...
            if hmcHost.hmcSoftware.versionInformation:
                ...
                if hmcHost.hmcSoftware.versionInformation.shortVersion:
                    hostOsh.setStringAttribute('discovered_os_version', hmcHost.hmcSoftware.versionInformation.shortVersion)
...
def createHmcSoftware(hmcSoftware, hostOsh):
    ...
    """
    if hmcSoftware:
        hmcOsh = modeling.createApplicationOSH('ibm_hmc', 'IBM HMC', hostOsh, 'virtualization', 'ibm_corp')
        if hmcSoftware.bios:
...
def doHmcSideInformation(shell, shellOsh, ip, reportHostName):
    """
...
#    discoverLparProfiles(shell, managedSystemsList)
#    discoverPartitionsIps(shell, managedSystemsList)
#    discoverProcessorPools(shell, managedSystemsList)
#    discoverScsi(shell, managedSystemsList)
 #   dsicoverVEthernetAdapters(shell, managedSystemsList)
...
#                        virtualHostOsh = modeling.createHostOSH(lpar.lparProfile.lparIpAddress, 'host', lpar.osName)
                        ...
                        if lpar.lparProfile.lparIpAddress:
                            virtualHostOsh = modeling.createHostOSH(lpar.lparProfile.lparIpAddress, 'host')
                        else:
                            virtualHostOsh = ObjectStateHolder('node')
...
def doDiscovery(Framework, shellUtils, ip, credentialId, codepage, shellName, warningsList, errorsList):
    ...
    try:
        try:
            ...
            try:
                if ibm_hmc_lib.isVio(shellUtils):
                    ...
                    return result
                result = doHmcSideInformation(shellUtils, shellOsh, ip, reportHostName)
...
def getShellUtils(Framework, shellName, credentials, ip, codepage, warningsList, errorsList):
    ...
    for credentialId in credentials:
        try:
            port = None
            protocol = ProtocolManager.getProtocolById(credentialId)
            ...
            shellUtils = shellutils.ShellUtils(client, None, protocolName)
            if shellUtils:
