...
##############################################
# modified to fix issues with not searching the registry properly for listening ports - Daniel La (JDS) 25/06/2014.
    ...
    try:
        ...
        if installedInstances:
            ...
            if installedInstanceNames.find('\n') > 0:
                ...
                installedIstanceNameList = installedInstanceNames.split('\n')
            else:
                installedIstanceNameList = [installedInstanceNames]
            #installedIstanceNameList = ['MSSQLSERVER', 'SQLEXPRESS']
        ...
        sqlServerDetailKeypaths = ['SOFTWARE\\Microsoft\\Microsoft SQL Server\\sHoRt_iNsTaNcEnAmE\\MSSQLServer\\SuperSocketNetLib\\Tcp', 'SOFTWARE\\Microsoft\\Microsoft SQL Server\\iNsTaNcEnAmE\\Setup', 'SOFTWARE\\Microsoft\\Microsoft SQL Server\\iNsTaNcEnAmE\\MSSQLServer\\CurrentVersion', 'SOFTWARE\\Microsoft\\Microsoft SQL Server\\iNsTaNcEnAmE\\Cluster', 'SOFTWARE\\Microsoft\\Microsoft SQL Server\\iNsTaNcEnAmE\\Cluster']
        sqlServerDetailFilters = ['TcpPort', 'SQLPath', 'CurrentVersion', 'ClusterIpAddr', 'ClusterName']
        for instanceName in instanceNameList:
            shortInstanceName = instanceName
            if instanceName.find('.') > 0:
                shortInstanceName = instanceName[instanceName.find('.')+1:]
            if shortInstanceName and shortInstanceName!= 'MSSQLSERVER' and str.isdigit(str(shortInstanceName)):
                dbconnect_utils.debugPrint(3, '[' + SCRIPT_NAME + ':registryLookup] Skipping short instance name <%s>' % shortInstanceName)
                continue
            ...
            for sqlServerDetailIndex in range(len(sqlServerDetailKeypaths)):
                ...
                if instanceName == 'MSSQLSERVER':
                    if sqlServerDetailKeypaths[sqlServerDetailIndex].find('sHoRt_') > 0:
                        sqlServerDetailKeypath = string.replace(sqlServerDetailKeypaths[sqlServerDetailIndex], 'Microsoft SQL Server\\sHoRt_iNsTaNcEnAmE', 'MSSQLServer')
                    else:
                    ...
                    if sqlServerDetailKeypaths[sqlServerDetailIndex].find('sHoRt_') > 0:
                        sqlServerDetailKeypath = string.replace(sqlServerDetailKeypaths[sqlServerDetailIndex], 'sHoRt_iNsTaNcEnAmE', shortInstanceName)
                    else:
                        sqlServerDetailKeypath = string.replace(sqlServerDetailKeypaths[sqlServerDetailIndex], 'iNsTaNcEnAmE', instanceName)
                if instanceName.find('MSSQL10_50') and sqlServerDetailKeypath.find('MSSQLSERVER\\MSSQLServer'):
                    sqlServerDetailKeypath = string.replace(sqlServerDetailKeypath,'MSSQLSERVER\\MSSQLServer','MSSQLSERVER' + instanceName)
            ...
            if instanceName == 'MSSQLSERVER':
                ...
                instanceName = dbconnect_utils.getServerName(localClient) + '\\' + instanceName
...
# Original registryLookup - drop the "_orig" if need to revert back to original - Daniel La (JDS) 25/06/2014
def registryLookup_orig(procToPortDict, dbInstanceDict, localClient, wmiRegistryClient):
    dbconnect_utils.debugPrint(3, '[' + SCRIPT_NAME + ':registryLookup] In registryLookup')
    try:
        ## Locals
        instanceNameList = []
        # If  SQL Server is present on this box, get instance names
        ...
        installedInstances = dbconnect_utils.getRegValues(localClient, wmiRegistryClient, installedInstancesKeypath, 'InstalledInstances')
        if installedInstances == None or str(installedInstances) == '[[], []]' or str(installedInstances) == '{}':
            # ALVARO. Before giving up, lets look for already discovered instancenames:
            if dbInstanceDict != None and len(dbInstanceDict) > 0:
                instancesString = ''
                for dbName in dbInstanceDict.keys():
                    instancesString = instancesString + dbName.upper() + '\n'
            ...
            if installedInstanceNames.find('\n') > 0:
                ...
                for installedInstanceName in installedIstanceNameList:
                    dbconnect_utils.debugPrint(3, '[' + SCRIPT_NAME + ':registryLookup] Found SQL Server instance <%s>' % installedInstanceName.strip())
                    installedInstanceRegNameKeyPath = 'SOFTWARE\\Microsoft\\Microsoft SQL Server\\Instance Names\\SQL'
                    installedInstanceRegNameDict = dbconnect_utils.getRegValues(localClient, wmiRegistryClient, installedInstanceRegNameKeyPath, installedInstanceName)
                    installedInstanceRegName = installedInstanceRegNameDict[installedInstanceRegNameKeyPath]
                    if installedInstanceRegName:
                        dbconnect_utils.debugPrint(3, '[' + SCRIPT_NAME + ':registryLookup] Found registry name <%s> for SQL instance name <%s>' % (installedInstanceRegName, installedInstanceName))
                        instanceNameList.append(installedInstanceRegName.strip())
                    else:
                        logger.debug("** appending: ", installedInstanceName.strip())
                        instanceNameList.append(installedInstanceName.strip())
            else:
                dbconnect_utils.debugPrint(3, '[' + SCRIPT_NAME + ':registryLookup] Found SQL Server instance <%s>' % installedInstanceNames.strip())
                installedInstanceRegNameKeyPath = 'SOFTWARE\\Microsoft\\Microsoft SQL Server\\Instance Names\\SQL'
                installedInstanceRegNameDict = dbconnect_utils.getRegValues(localClient, wmiRegistryClient, installedInstanceRegNameKeyPath, installedInstanceNames.strip())
                installedInstanceRegName = installedInstanceRegNameDict[installedInstanceRegNameKeyPath]
                if installedInstanceRegName and not installedInstanceNames == 'MSSQLSERVER':
                    dbconnect_utils.debugPrint(3, '[' + SCRIPT_NAME + ':registryLookup] Found registry name <%s> for SQL instance name <%s>' % (installedInstanceRegName, installedInstanceNames.strip()))
                    instanceNameList.append(installedInstanceRegName.strip())
                else:
                    instanceNameList.append(installedInstanceNames.strip())
        ...
        for instanceName in instanceNameList:
            logger.debug("** in for loop for instanceName: ", instanceName)
            shortInstanceName = instanceName
            if instanceName.find('.') > 0:
                shortInstanceName = instanceName[instanceName.find('.')+1:]
            if shortInstanceName and shortInstanceName.upper()!= 'MSSQLSERVER' and type(eval(shortInstanceName)) == type(1):
                dbconnect_utils.debugPrint(3, '[' + SCRIPT_NAME + ':registryLookup] Skipping short instance name <%s>' % shortInstanceName)
                continue
            sqlServerDetailValues = []
            for sqlServerDetailIndex in range(len(sqlServerDetailKeypaths)):
            ...
            if instanceName == 'MSSQLSERVER':
                ...
                instanceName = shortInstanceName.lower()
                instanceName = dbconnect_utils.getServerName(localClient) + '\\' + instanceName
            installPath = installPath.lower()
            if instanceName in dbInstanceDict.keys():
                statusFlag = (dbInstanceDict[instanceName])[dbconnect_utils.STATUS_INDEX]
                ...
                dbconnect_utils.debugPrint(3, '[' + SCRIPT_NAME + ':registryLookup] Added SQL Server <%s> instance <%s> listening at port <%s> on <%s> and installed in <%s>' % (version, instanceName, listenerPort, ipAddress, installPath))
            ## Replace dictionary entry of serverName\sqlInstanceName with clusterName\sqlInstanceName
            if clusterName and instanceName.find('\\')>0 and instanceName in dbInstanceDict.keys():
